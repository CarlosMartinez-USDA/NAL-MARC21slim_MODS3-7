...
-->
            ...
            <xsl:if test="
                ...
                <relatedItem>
                    ...
                    <xsl:for-each
                        ...
                        <!-- identifiers -->
                        <xsl:apply-templates select="subfield[@code = 'o']" mode="relatedItemOf"/>
                        <xsl:apply-templates select="subfield[@code = 'x']" mode="relatedItemX"/>
                        <xsl:apply-templates select="datafield[@tag = '914']/subfield[@code = 'a']"
                            mode="relatedItem914"/>
                        <!--  1.120 - @76X-78X$z -->
                        <xsl:apply-templates select="subfield[@code = 'z']" mode="relatedItem"/>
    ...
    <xsl:template name="personal_name">     
        ...
        <xsl:for-each select="$name">
            <namePart type="given">
                    <xsl:analyze-string select="substring-after($name,',')" regex="'([A-Z]+$)|([A-Z]\.[A-Z]\.)$|([A-z]+\s?[A-z]\.?$)|([A-z]+[A-z]\.?)$'">
                        <xsl:matching-substring>
                            <xsl:choose>
                                <xsl:when test="regex-group(1)">
                                    <xsl:value-of select="f:capitalize-first(regex-group(1))"/>
                                </xsl:when>
                                <xsl:when test="regex-group(2)">
                                   <xsl:value-of select="upper-case(regex-group(2))"/> 
                                </xsl:when>
                                <xsl:when test="regex-group(3)">
                                    <xsl:value-of select="f:proper-case(regex-group(3))"/>
                                </xsl:when>
                   ...
                   <xsl:non-matching-substring>
                       <xsl:choose>
                           <!-- initials only -->
                           <xsl:when
                               test="matches(translate($name,',. ',''), '([A-Z]\.$|[A-Z]\.(\s)?[A-Z]\.$)')">
                               <xsl:value-of 
                                   select="($name)"/>
                           </xsl:when>
                           <!-- first name-->
                           <xsl:otherwise>
                               <xsl:value-of
                                   select="f:proper-case(replace($name, '\w+$|\w+\.\s?[A-Z]\.$', '$2'))"
                               />
                           </xsl:otherwise>
                       </xsl:choose>
                   </xsl:non-matching-substring>
        ...
        <xsl:if test="datafield[@tag='100' or @tag='700']/subfield[@code='1']">
            <nameIdentifier>
                <xsl:choose>
    ...
    <xsl:template name="nameIdAttr">
        <xsl:param name="selectType" select="../subfield[@code='1']" tunnel="yes"/>
        <xsl:attribute name="type">
    ...
    <xsl:template match="//datafield[@tag='100' or @tag='700']/subfield[@code='1']" mode="name_authority">
