...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="2.0"
	...
	Revision 1.167 - Custom function f:lookupMARCCountry($marcCode) returns the country/state name
...
-->
            ...
            <xsl:if test="
                ...
                <relatedItem>
                    ...
                    <xsl:for-each
                        ...
                        <xsl:apply-templates
                            select="//datafield[@tag = '914']/subfield[@code = 'a']"
                            mode="relatedItem914"/>
                        <!--  related part  -->
                        <xsl:if test="@tag = '773'">
        ...
        <!--NAL notes 910, 930, 945, 946, ('974': Removed and put with local identifers) -->
        <extension>
...
&lt;/xsl:template&gt;-->
    ...
    <!--<xd:doc>
        <xd:desc>@deprcated -\- no longer used</xd:desc>
    </xd:doc>
    <xsl:template name="createNoteFrom974" match="//datafield[@tag = '974']">
        <!-\- Revision 1.159 added if test to prevent extra whitespace -\->
      	...
      	<xsl:if test="//datafield[@tag = 974]/subfield[@code = 'b']">
      		  <xsl:text>&#160;</xsl:text>
      		  <idenfitifer type="local">
      		  <xsl:value-of select="normalize-space(datafield[@tag = 974]/subfield[@code = 'b'])"/>
      		  </idenfitifer>
      	</xsl:if>
    </xsl:template>
-->
                    ...
                    <xsl:non-matching-substring>
                        ...
                        <xsl:if test="matches($marcPublicationCode, '\D{3,4}')">
                            <place>
                                ...
                                <placeTerm>
                                    <xsl:attribute name="type">text</xsl:attribute>
                                    <xsl:value-of select="f:lookupMARCCountry($marcPublicationCode)"
                                    />
                                </placeTerm>
                            </place>
                        </xsl:if>
