...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="2.0"
	...
	Revision 1.150 - updated marc:recordOrigin to reflect the XSLT filename used in transform. cm3 2022/12/08
	...
	Revision 1.44 - Added <marc:recordInfo><marc:recordOrigin> canned text about the version of this stylesheet 2009 ntra
    ...
    <xsl:template name="marcRecord">
                ...
                <xsl:variable name="title">
                    <xsl:call-template name="chopPunctuation">
                        <xsl:with-param name="chopString">
                            <xsl:call-template name="mSubfieldSelect">
                                <!-- 1/04 removed $h, b -->
		...
		<originInfo>
			...
			<xsl:if test="($controlField008-6='e' or $controlField008-6='p' or $controlField008-6='r' or $controlField008-6='s' or $controlField008-6='t') and ($marcLeader6='d' or $marcLeader6='f' or $marcLeader6='p' or $marcLeader6='t')">
				<xsl:if test="$controlField008-7-10 and ($controlField008-7-10 != $marc:datafield260c)">
                    ...
                    <xsl:when test="count(marc:subfield[@code = 'a']) &gt; 1">
                        <xsl:for-each select="marc:subfield[@code = 'a']">
                            ...
                            <xsl:if test="following-sibling::marc:subfield[@code = 'c']">
                                <xsl:for-each
                                    select="following-sibling::marc:subfield[@code = 'c'][1]">
                                    <publisher>
                                        <xsl:value-of select="substring-before(., ',')"/>
                                    </publisher>
                                </xsl:for-each>
...
-->
                    ...
                    <xsl:for-each
                        ...
                        <titleInfo>
                            ...
                            <title>
                                <xsl:call-template name="chopPunctuation">
                                    <xsl:with-param name="chopString">
                                        <xsl:call-template name="mSpecialSubfieldSelect">
                        ...
                        <!-- 1.164 removed mSpecialSubfieldSelect template -->
                        <name type="personal">
                            <xsl:call-template name="xxx880"/>
                    ...
                    <xsl:for-each
                        ...
                        <xsl:for-each select="marc:subfield[@code = 't']">
                            <titleInfo>
                                ...
                                <xsl:if
                                    test="parent::*[@tag != 773] and ../marc:subfield[@code = 'g']">
                                ...
                                <part>
                                    ...
                                    <xsl:analyze-string select="." regex="^(\d+\D+)(\d+)(.*)$">
                                        <xsl:matching-substring>
                                            <!-- volume -->
                                            <xsl:choose>
                                                <xsl:when test="contains(regex-group(2), 'v.')"> </xsl:when>

                                                <xsl:when test="contains(regex-group(3), 'v.')"
                                                > </xsl:when>
                                            </xsl:choose>
                                            <detail type="volume">
                                                <number>
        ...
        <!-- 1.162 NAL control number mapped to identifier from 001 -->
        ...
        <!--NAL notes 910, 930, 945, 946, (974 moved to list of idenitiers) -->
        <extension>
    ...
    <!--1.165 remnamed displayForm template, added mSpecialSubfieldSelect from 1.164 as variable, parsed it as output-->
        ...
        <xsl:for-each select="$whole_name">
            <namePart type="given">
                <xsl:choose>
                    ...
                    <xsl:otherwise>
                        <xsl:value-of
                            select="normalize-space(replace(substring-after($whole_name, ','), '(\s)(.*)(,|\.)', '$2'))"
                        />
                    </xsl:otherwise>
                </xsl:choose>
            </namePart>
            ...
            <namePart type="family">
                <xsl:value-of select="substring-before(normalize-space($whole_name), ',')"/>
            </namePart>
            <displayForm>
                <xsl:choose>
                	<!--last_name, initals only-->
                    <xsl:when test="matches(substring-after(., ','), '([A-Z]\.$|[A-Z]\.[A-Z]\.$)')">
                        <xsl:value-of select="$whole_name"/>
                    </xsl:when>
                	<!-- last_name, first_name -->
                    <xsl:otherwise>
                        <xsl:value-of select="replace($whole_name, '(.*,.*)(,|\.)', '$1')"/>
                    </xsl:otherwise>
                </xsl:choose>
            </displayForm>
        </xsl:for-each>
    ...
    <xsl:template name="relatedIdentifierISSN">
        <xsl:choose>
            ...
            <xsl:when test="../marc:datafield[@tag = 914]/marc:subfield[@code = 'b'] != ''">
                <xsl:for-each select="../marc:datafield[@tag = 914]/marc:subfield[@code = 'b']">
                    <identifier type="issn">
                        <xsl:value-of select="."/>
                    </identifier>
                </xsl:for-each>
    ...
    <xsl:template name="relatedIdentifierLocal">
        <xsl:choose>
            ...
            <xsl:otherwise>
                <xsl:for-each select="../marc:datafield[@tag = 914]/marc:subfield[@code = 'a']">
                    <identifier type="local">
                        <xsl:value-of select="."/>
                    </identifier>
                </xsl:for-each>
    ...
    <xsl:template name="mSpecialSubfieldSelect">
...
&lt;/xsl:template&gt;-->
                ...
                <temporal encoding="iso8601" point="start">
                    <xsl:call-template name="dates045b">
                        <xsl:with-param name="str"
                            select="marc:subfield[@code = 'b' or @code = 'c'][1]"/>
                    </xsl:call-template>
                </temporal>
                <temporal encoding="iso8601" point="end">
                    <xsl:call-template name="dates045b">
                        <xsl:with-param name="str"
                            select="marc:subfield[@code = 'b' or @code = 'c'][2]"/>
                    </xsl:call-template>
                </temporal>
            </subject>
        </xsl:if>
    ...
    <xsl:template name="createNoteFrom946"
        match="marc:datafield[@tag = 946]/marc:subfield[@code = 'a']">
    ...
    <xsl:template name="createNoteFrom974"
        match="marc:datafield[@tag = 974]/marc:subfield[@code = 'a']">
        <!-- Revision 1.159 added if test to prevent extra whitespace -->
        <xsl:if test="marc:datafield[@tag = 974]">
            <identifier type="local">
                <!-- $a agid: -->
                <xsl:value-of
                    select="normalize-space(marc:datafield[@tag = 974]/marc:subfield[@code = 'a'])"/>
                <!-- if test for $b avoids excess whitespace within the field -->
                <xsl:if test="count(marc:datafield[@tag = 974]/marc:subfield[@code = 'b']) != 0">
                    <xsl:text>&#160;</xsl:text>
                    <xsl:value-of
                        select="normalize-space(marc:datafield[@tag = 974]/marc:subfield[@code = 'b'])"
                    />
                </xsl:if>
            </identifier>
        </xsl:if>
    ...
    <xd:doc id="createClassificationFrom070" scope="component">
        <xd:desc>createClassificationFrom070</xd:desc>
    </xd:doc>
        ...
        <location>
            ...
            <xsl:if test="marc:subfield[@code = 'p' or @code = 't']">
                <holdingSimple>
                    <copyInformation>
                        <xsl:for-each
                            select="marc:subfield[@code = 'p'] | marc:subfield[@code = 't']">
                            <itemIdentifier>
                                <xsl:if test="@code = 't'">
                                    <xsl:attribute name="type">
    ...
    <xsl:template
        match="marc:datafield[@tag = '856']/marc:subfielssd[@code = 'q'] | marc:datafield[@tag = '880'][starts-with(marc:subfield[@code = '6'], '856')]/child::*[code = 'q']"
        mode="physDesc">
    ...
    <xd:doc id="reformattingQuality" scope="component">
