...
<xd:doc scope="component">
	...
	<xsl:function name="f:name-identifier" as="xs:token">
		<xsl:param name="arg" as="xs:token"/>
		<xsl:variable name="varg" select="tokenize($arg, distinct-values($arg))"/>

	<xsl:choose>
	<xsl:when test="count($varg)>1">
	<xsl:sequence select="if (contains($varg,'orcid|viaf|isni|[a-z]+')=true())
						  then replace($varg,'(^https?)://(www)?(\w+)((\.\w+)(\.\w+)?(\.\w+)?)/?(\S+)/?(\?uri=)?(.*)','$3')
						  else if (matches($varg,'id.loc.gov|id.nlm.nih.gov|agclass.nal.usda.gov|lod.nal.usda.gov|[a-z]\.[a-z]\.gov|[a-z]\.[a-z]\.org'))
						  then replace($varg,'(^https?)://(www)?(\w+)((\.\w+)(\.\w+)?(\.\w+)?)/?(\S+)/?(\?uri=)?(.*)','$3$4')
						  else $varg"/>
	</xsl:when>
	<xsl:otherwise>
  	<xsl:value-of select="if (matches($varg,'orcid|viaf|isni|[a-z]+'))
							then replace($varg,'(^https?)://(www)?(\w+)((\.\w+)(\.\w+)?(\.\w+)?)/?(\S+)/?(\?uri=)?(.*)','$3')
							else if (matches($varg,'id.loc.gov|id.nlm.nih.gov|agclass.nal.usda.gov|lod.nal.usda.gov|[a-z]\.[a-z]\.gov|[a-z]\.[a-z]\.org'))
							then replace($varg,'(^https?)://(www)?(\w+)((\.\w+)(\.\w+)?(\.\w+)?)/?(\S+)/?(\?uri=)?(.*)','$3$4')
							else $varg"/>
	...
	<xsl:function name="f:tokenize-arg" as="item()*">
			<xsl:param name="arg" as="item()"/>
			<xsl:for-each select="$arg">
				<xsl:sequence select="tokenize($arg,distinct-values($arg))"/>
			</xsl:for-each>			
		</xsl:function>
