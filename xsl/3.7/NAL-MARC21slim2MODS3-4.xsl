...
<!--    <xsl:output encoding="UTF-8" indent="yes" method="xml" name="saxon" saxon:next-in-chain="../fix_characters.xsl"/>-->
...
<!--    <xsl:output encoding="UTF-8" indent="yes" method="xml" name="saxon" saxon:next-in-chain="fix_characters.xsl"/>-->
    ...
    <xd:doc>
        <xd:desc>normalizes whitespace amongst text()</xd:desc>
    </xd:doc>
    <xsl:template match="node()|@*|text()">
        <xsl:for-each select="*">
        <xsl:value-of select="normalize-space(node()|@*) and normalize-space(text())"/>
        </xsl:for-each>
    </xsl:template>
...
<!--href="file:///{$workingDir}N-{replace($originalFilename,'(.*/)(.*)(\.xml)','$2')}_{position()}.xml">-->
    ...
    <xsl:template match="/">
        <xsl:result-document encoding="UTF-8" version="1.0" method="xml" media-type="text/xml"
            ...
            href="{replace(saxon:system-id(),'(.*/)(.*)(\.xml)','$1')}N-{replace(saxon:system-id(),'(.*/)(.*)(\.xml)','$2')}_{position()}_v3-4.xml">
                    <xsl:choose>
               <xsl:when test="//collection/record">
                    <collection xmlns="http://www.loc.gov/mods/v3"
                        ...
                        <xsl:if test="matches($marcPublicationCode, '\D{3,4}')">
                            <place>
                                ...
                                <placeTerm>
                                    ...
                                    <xsl:value-of
                                        select="f:decodeMARCCountry($marcPublicationCode)"/>
                                </placeTerm>
                            </place>
                        </xsl:if>
                    </xsl:non-matching-substring>
...
-->
    ...
    <xsl:template name="relatedIdentifierISSN" match="subfield[@code='x']" mode="relatedItem">
          ...
          <xd:doc>
            ...
            <xsl:template match="datafield[@tag = 914]/subfield[@code = 'a']">
