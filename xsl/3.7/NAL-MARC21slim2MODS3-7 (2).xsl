...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="2.0"
	...
	│ MODS 3.7 │  
	└ ━ ━ ━ ━ ━ ┘
	Revision 1.169 - Used variable $this to encapsulate fields containing longer text content, in order to prevent schema whitespacing error. <2></2>0230108 cm3
...
-->
        ...
        <xsl:for-each select="
            ...
            datafield[@tag = '785'] | datafield[@tag = '786'] | datafield[@tag = '914'] |
            ...
            datafield[@tag = '880'][starts-with(subfield[@code = '6'], '777')] | datafield[@tag = '880'][starts-with(subfield[@code = '6'], '780')] | 
            datafield[@tag = '880'][starts-with(subfield[@code = '6'], '785')] | datafield[@tag = '880'][starts-with(subfield[@code = '6'], '786')] |
            datafield[@tag = '880'][starts-with(subfield[@code = '6'], '914')]">
            <xsl:variable name="s6" select="substring(normalize-space(subfield[@code = '6']), 5, 2)"/>
            <xsl:variable name="tag" select="@tag"/>
            <!-- selects type attribute -->
            <xsl:if test="(@tag = 760 or @tag = 762 or @tag = 765 or @tag = 767 or @tag = 770 or @tag = 774 or @tag = 775 or @tag = 772 or @tag = 773 
                or @tag = 776 or @tag = 777 or @tag = 780 or @tag = 785 or @tag = 786) or (@tag='880' and not(../marc:datafield[@tag = 760 
                or @tag = 762 or @tag = 765 or @tag = 767 or @tag = 770 or @tag = 774 or @tag = 775 or @tag = 772 or @tag = 773 or @tag = 776 or @tag = 777 
                or @tag = 780 or @tag = 785 or @tag = 786 or @tag = 914][substring(marc:subfield[@code='6'],5,2) = $s6]))">
                <relatedItem>
                    <!--  selects type attribute  -->
                    <xsl:choose>
                        <!-- 1.120 - @762@type -->
...
<!--                        <xsl:apply-templates select="subfield[@code = 'x']" mode="relatedItem"/>-->
                        <xsl:apply-templates select="datafield[@tag='773'][subfield[@code = 'x']]" mode="relatedItem"/>
                        <!--   1.120 - @76X-78X$z  -->
                        <xsl:apply-templates select="subfield[@code = 'z']" mode="relatedItem"/>
                        <xsl:apply-templates select="datafield[@tag='773'][subfield[@code = 'w']]" mode="relatedItem"/>
...
-->
    ...
    <xsl:template match="datafield[@tag = '773'][subfield[@code='x' or @code='w']]" mode="relatedItem">
        <xsl:param name="issn"/>
        ...
        <identifier type="issn">
            ...
            <xsl:apply-templates>
                ...
                <xsl:with-param name="local">
                    <xsl:copy-of select="../subfield[@code = 'w']"/>
                </xsl:with-param>
            </xsl:apply-templates>
        </identifier>
    </xsl:template>
    ...
    <xd:doc>
        <xd:desc> ISSN (914$b) is mapped into [identifier type="issn]</xd:desc>
    </xd:doc>
...
&lt;/xsl:template&gt;-->
    ...
    <xsl:template name="originInfo">
        ...
        <xsl:variable name="originInfoShared">
            <!--decodes 2 or 3 letter MARC Country Codes -->
            ...
            <xsl:choose>
                ...
                <xsl:when
                    ...
                    <!--uses controlfield008 date1/date2 information and results in w3cdtf format-->
...
<!--
    <xd:doc>
        <xd:desc>Remove extraneous 787/relatedItem fields</xd:desc>
    </xd:doc>
    <xsl:template match="datafield[@tag = '787']/subfield[@code = 'o']">
   <!-\-empty template to remove extrandoues relatedItem fields-\-> 
</xsl:template>-->
</xsl:stylesheet>
