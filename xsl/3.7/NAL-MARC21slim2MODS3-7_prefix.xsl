...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="2.0"
    ...
    <xsl:param name="workingDir" select="substring-before(saxon:system-id(),tokenize(base-uri(),'/')[last()])"/>
 ...
 2009/11/23 tmee
	...
	Revision 1.24 - Mapped 852 subfields $h, $i, $j, $k, $l, $m, $t to <shelfLocation> and 852 marc:subfield $u to <physicalLocation> with @xlink 2008/09/17 tmee
...
-->
        ...
        <!-- 1.120(@777 @787) , 1.121, 1.168(@914$a)  -->
        <xsl:for-each
            ...
            <xsl:if test="(@tag = 760 or @tag = 762 or @tag = 765 or @tag = 767 or @tag = 770 or @tag = 774 or 
                ...
                <relatedItem>
                    ...
                    <xsl:for-each
                        ...
                        <xsl:apply-templates select="marc:datafield[@tag = '914']/marc:subfield[@code = 'a']" mode="relatedItem914"/>
                        <!--  1.120 - @76X-78X$z -->
                        <xsl:apply-templates select="marc:subfield[@code='z']" mode="relatedItem"/>
    ...
    <xsl:template name="personal_name">
        ...
        <xsl:for-each select="$whole_name">
            <namePart type="given">
                <xsl:choose>
                    <!-- initials only -->
                    <xsl:when
                        test="matches(substring-after($whole_name, ','), '([A-Z]\.$|[A-Z]\.[A-Z]\.$)')">
                        <xsl:value-of select="f:proper-case(normalize-space(substring-after($whole_name, ',')))"/>
                    </xsl:when>
                    <!-- first name-->
                    <xsl:otherwise>
                        <xsl:value-of
                            select="f:proper-case(normalize-space(replace(substring-after($whole_name, ','), '(\s)(.*)(,|\.)', '$2')))"
                        />
                    </xsl:otherwise>
                </xsl:choose>
            </namePart>
            ...
            <namePart type="family">
                <xsl:value-of select="f:proper-case(substring-before(normalize-space($whole_name), ','))"/>
            </namePart>
            <displayForm>
                <xsl:choose>
                    <!--last_name, initals only-->
                    <xsl:when test="matches(substring-after(., ','), '([A-Z]\.$|[A-Z]\.[A-Z]\.$)')">
                        <xsl:value-of select="f:proper-case($whole_name)"/>
                    </xsl:when>
                    <!-- last_name, first_name -->
                    <xsl:otherwise>
                        <xsl:value-of select="replace(f:proper-case($whole_name), '(.*,.*)(,|\.)', '$1')"/>
                    </xsl:otherwise>
                </xsl:choose>
            </displayForm>
        </xsl:for-each>
    ...
    <xsl:template name="originInfo">
        ...
        <xsl:variable name="originInfoShared">
            ...
            <xsl:variable name="marc:controlfield008-7-12">
                <xsl:analyze-string select="replace($controlField008,'(\d+[a-z])(\d+.*)','$2')" regex="(\d+)(.*)">
                    <xsl:matching-substring>
                        <xsl:value-of select="regex-group(1)"/>
                    </xsl:matching-substring>
                    <xsl:non-matching-substring>
