...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="2.0"
	...
	Revision 1.94 - Leader 07 b mapping changed from "continuing" to "serial" tmee 2014/02/21d
 ...
 2009/11/23 tmee
	...
	-->
    <!--Result document parameters for development, testing and production -->   
    <!--1. dev: href="{replace(saxon:system-id(),'(.*/)(.*)(\.xml)','$1')}N-{replace(saxon:system-id(),'(.*/)(.*)(\.xml)','$2')}_{position()}.xml">-->
    <!--2.test: href="{replace(saxon:system-id(),'(.*/)(.*)(\.xml)','$1')}N-{replace(saxon:system-id(),'(.*/)(.*)(\.xml)','$2')}_{position()}_v3-7.xml">-->
    <!--3.prod: href="file:///{$workingDir}N-{replace($originalFilename,'(.*/)(.*)(\.xml)','$2')}_{position()}.xml">-->
    ...
    <xsl:template name="marcRecord">
                ...
                <xsl:choose>
                    ...
                    <xsl:when test="@ind2 != ' ' and @ind2 &gt; 0">
                        ...
                        <nonSort xml:space="preserve"><xsl:value-of select="substring($titleChop, 1, @ind2)"/> </nonSort>
                            <xsl:value-of select="normalize-space(substring($titleChop, @ind2 + 1))"/>                    </xsl:when>
                    <xsl:otherwise>                    
        ...
        <!-- removed deprecated <originInfo> template 20220108 cm3-->
        ...
        <!-- 1.121 -->
        <xsl:for-each
            select="datafield[@tag = 'f'] | datafield[@tag = '880'][starts-with(subfield[@code = '6'], '500')]">
            ...
            <xsl:if
                ...
                <relatedItem type="original">
                    <xsl:for-each
                        ...
                        <!-- related item id -->
                        <xsl:apply-templates select="subfield[@code = 'x']" mode="relatedItemX"/>
        ...
        <!-- 1.120: @777 @787 // 1.121 // 1.168: @914 and @787-removal  -->
        <!-- 1.120 - @777 @787 and 1.121 -->
        <xsl:for-each
            select="datafield[@tag='760'] | datafield[@tag='762'] | datafield[@tag='765'] | 
            datafield[@tag='767'] | datafield[@tag='770'] | datafield[@tag='774'] | 
            datafield[@tag='775'] | datafield[@tag='772'] | datafield[@tag='773'] |
            datafield[@tag='776'] | datafield[@tag='777'] | datafield[@tag='780'] |
            datafield[@tag='785'] | datafield[@tag='786'] | datafield[@tag='787'] |
            datafield[@tag='914'] |
            ...
            datafield[@tag='880'][starts-with(subfield[@code='6'],'777')] | datafield[@tag='880'][starts-with(subfield[@code='6'],'780')] | datafield[@tag='880'][starts-with(subfield[@code='6'],'785')] |
            datafield[@tag='880'][starts-with(subfield[@code='6'],'786')] | datafield[@tag='880'][starts-with(subfield[@code='6'],'787')] |
            datafield[@tag='880'][starts-with(subfield[@code='6'],'914')]">
            <xsl:variable name="s6" select="substring(normalize-space(subfield[@code='6']), 5, 2)"/>
            ...
            <xsl:if test="(@tag = 760 or @tag = 762 or @tag = 765 or @tag = 767 or @tag = 770 or @tag = 774 or 
                ...
                <relatedItem>
                    ...
                    <xsl:for-each
                        ...
                        <!-- identifiers -->
                        <xsl:apply-templates select="subfield[@code='x']" mode="relatedItemX"/>
                        <xsl:apply-templates select="datafield[@tag = '914']/subfield[@code = 'a']" mode="relatedItem914"/>
                        <!--  1.120 - @76X-78X$z -->
        ...
        <!--@Libary of Congress' related part -->
      <!--  <xsl:if test="@tag='773'">
        ...
        <xsl:for-each select="$whole_name">
            <namePart type="given">
                <xsl:choose>
                    ...
                    <xsl:when
                        ...
                        <xsl:value-of select="f:proper-case(normalize-space(substring-after(., ',')))"/>
                    </xsl:when>
                    <!-- first name-->
                    <xsl:otherwise>
                        <xsl:value-of
            ...
            <displayForm>
                <xsl:choose>
                    ...
                    <xsl:otherwise>
                        <xsl:value-of select="f:proper-case(replace($whole_name, '(.*,.*)(,|\.)', '$1'))"/>
                    </xsl:otherwise>
                </xsl:choose>
            </displayForm>
        </xsl:for-each>
...
-->
    ...
    <xsl:template match="subfield[@code = 'w']" mode="relatedItemW">
        <identifier type="local">
             <!-- 1.121 -->
             <xsl:call-template name="xxs880"/>
             <xsl:apply-templates/>
   ...
   <!-- <xsl:template name="issnLocal" match="*[@tag = '773']">
        <xsl:param name="issn"/>
	...
	<xd:doc id="relatedItem914" scope="component">
		<xd:desc>datafield[@tag = '914']/subfield[@code = 'b']: ISSN (914$b) is mapped into [identifier type="issn]</xd:desc>
	</xd:doc>
	<xsl:template match="datafield[@tag = '914']/subfield[@code = 'b']" mode="relatedItem914">
        <identifier type="issn">
            <!-- 1.121 -->
            <xsl:call-template name="xxs880"/>
            <xsl:apply-templates/>
...
-->
    <!-- 1.121 -->
	<xd:doc id="relatedItemNote" scope="component">
		<xd:desc>subfield[@code = 'n']: Creates related item notes </xd:desc>
	</xd:doc>
    ...
    <!-- 1.121 -->
	<xd:doc id="relatedItem" scope="component">
		<xd:desc>subfield[@code = 'h']: Creates related item form </xd:desc>
	</xd:doc>
    ...
    <!-- 1.121 -->
	<xd:doc id="relatedItem" scope="component">
		<xd:desc>subfield[@code = 'j']: Creates related item subjects </xd:desc>
	</xd:doc>
	...
	<xd:doc id="relatedItem" scope="component">
		<xd:desc>subfield[@code = 'a']: 1.121 Creates related item names </xd:desc>
	</xd:doc>
...
&lt;/xsl:template&gt;-->
	...
	<xd:doc id="global_copy" scope="component">
	...
	<xd:doc id="physDesc" scope="component">
		<xd:desc>datafield[@tag = '351']: 351 note</xd:desc>
	</xd:doc>
	...
	<xd:doc id="originInfo" scope="component">
		<xd:desc>subfield[@code = '6']: @880$6 </xd:desc>
	</xd:doc>
