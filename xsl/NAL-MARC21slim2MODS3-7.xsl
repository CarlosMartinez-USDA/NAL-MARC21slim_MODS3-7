...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="2.0"
	...
	│ MODS 3.7 │  
	└ ━ ━ ━ ━ ━ ┘
	Revision 1.170 - Added f$controlField008-0-14 variable to get correct dates is "w3cdtf" format. / 20230108, cm3
	...
	Revision 1.167 - Custom function f:decodeMARCCountry($marcCode) returns the country/state name. / 20230106,  cm3
	Revision 1.166 - Filters 008 field for 2 or 3 letter country/state codes. / 20230106,  cm3
	Revision 1.165 - Remnamed displayForm template, added SubfieldSelect from 1.164 as variable, parsed it as output. 20230106 cm3
	Revision 1.164 - Removed SubfieldSelect template. 20230106 cm3
	...
	Revision 1.158 - Added conditional statement above which only provides an <issuane> to monographs, single part items and multipart monographs /  12/08/2022, cm3 
	Revision 1.157 - Added condtional statement if agid:# is empty from 773, use 914 subfield a / 2022/12/09/2022 cm3 
	Revision 1.156 - Added condtional statement if ISSN is empty from 773, use 914 subfield b / 2022/12/09/ cm33 
    Revision 1.155 - Added custom function to map category code to subject. / 2022/12/08, cm3
    Revision 1.154 - Commented out conditional statements within <issuance> element to exclude serials, contininuing resources, and integrating resources from receving this tag.  / 2022/12/08, cm3 
    Revision 1.153 - Used subtring-before function to get subfield b (ie., publisher) and subfield c (i.e., dateIssued). cm3 2022/12/08
	Revision 1.152 - Added conditional statement outside of issuance element to allow monographs, multipart monographs, and single items only. / 2022/12/08, cm3
	Revision 1.151 - $, uses replace function and regex to capture 3 letter string.  2022/12/05, cm3
	Revision 1.150 - Updated recordOrigin to reflect the XSLT filename Used in transform. cm3 2022/12/08 cm3
	Revision 1.149 - Added if tests to originInfo producer elements to avoid empty tag cm3 2022/11/10, cm3
	Revision 1.148 - Used analyze-string function 008 to pull out 2-3 letter text and apply custom marc country conversion function. / 2022/10/05, cm3
	Revision 1.147 - Removed  prefix from XSLT to accomodate prefix-less elements from Alma 2022/10/05, cm3
	Revision 1.146 - Upgraded stylesheet to use XSLT 2.0 cm3 2022/10/05, cm3
	Revision 1.145 - Fixed dateIssued to include year only date from the 008 / 2014/12/16, JG 
	Revision 1.144 - Fixed dateIssued to include month and date from the 008 / 2014/08/18,  JG
	Revision 1.143 - Added displayForm to / 2014/05/29, JG 
	Revision 1.141 - Added subject from 072, classification from 070, identifier from 024, identifier from 001, and notes from 910, 930, 945, 946, 974, /  2014/04/23, NAL Staff
	┌ ━ ━ ━ ━ ━ ━ ━ ━ ━ ━ ━ ━ ━ ━ ━ ━ ━ ━ ━ ━ ━ ━ ━ ━ ┐ 
            ...
            <xsl:choose>
                ...
                <xsl:otherwise>
                    <mods xmlns:mods="http://www.loc.gov/mods/v3"
                        ...
                        xsi:schemaLocation="http://www.loc.gov/mods/v3 http://www.loc.gov/standards/mods/v3/mods-3-7.xsd"
                        version="3.7">
                        <xsl:for-each select="//record and not(//collection)">
                            <xsl:call-template name="marcRecord"/>
                        </xsl:for-each>
                    </mods>
                </xsl:otherwise>
...
-->
        ...
        <xsl:if test="$controlField008-35-37">
            <language>
                <languageTerm authority="iso639-2b" type="code">
                    <xsl:choose>
                        <xsl:when
                            test="ends-with($controlField008-35-37, 'ng') and not(contains($controlField008-35-37, 'eng'))">
                            <xsl:value-of select="string('eng')"/>
                        </xsl:when>
                        <xsl:when
                            test="matches($controlField008-35-37, '[a-z]{3}') and count($controlField008-35-37) != 0">
        ...
        <xsl:if
            ...
            <xsl:choose>
                ...
                <xsl:when test="$controlField008-22 = 'f'">
                    <targetAudience authority="marctarget">ized</targetAudience>
                </xsl:when>
            </xsl:choose>
        </xsl:if>
        ...
        <xsl:for-each select="
                ...
                datafield[@tag = '780'] | datafield[@tag = '785'] | datafield[@tag = '786'] | datafield[@tag = '914'] |
            ...
            <xsl:if test="
                ...
                <relatedItem>
                    <!-- selects mods:relateItem[@type] attribute -->
                    <xsl:choose>
                        <!-- 1.120 - @762@type -->
            ...
            <xsl:for-each select="datafield[@tag = '040']/subfield[@code = 'b']">
                <xsl:if test="count(subfield[@code = 'b']) != 0">
                    <languageOfCataloging>
                        ...
                        <languageTerm type="text">
                            <xsl:value-of select="f:isoTwo2Lang(.)"/>
                        </languageTerm>
                    </languageOfCataloging>
                </xsl:if>
            </xsl:for-each>
    ...
    <xsl:template name="part">
     ...
     <xd:doc id="relatedpart_773" scope="component">
    ...
    <xsl:template match="subfield[@code = 'x']" mode="relatedItemX">
        <identifier type="issn">
            <!-- 1.168 -->
            <xsl:call-template name="xxs880"/>
            <xsl:apply-templates/>
  ...
  <xd:doc id="relatedIdentifierLocal" scope="component">
    ...
    <xsl:template match="datafield[@tag = '914']/subfield[@code = 'a']" mode="relatedItem914">
        <!-- 1.168 -->
        <identifier type="local">
            <xsl:value-of select="../subfield[@code = 'a']"/>
        </identifier>
    ...
    <xsl:template match="subfield[@code = 'o']" mode="relatedItemOf"/>
...
-->
    ...
    <xsl:template name="nameIdentifier_zero">
    ...
    <xsl:template name="nameIdentifier_one">
        <xsl:if test="subfield[@code = '1']">
            <nameIdentifier>
                <xsl:call-template name="subfieldSelect">
    ...
    <xd:doc id="=subfield_code_1" scope="component">
        <xd:desc> 1.116 </xd:desc>
    </xd:doc>
    <xsl:template name="subfield_code_1">
        <!--orcID, viaf, lcnaf, etc. -->
        <xsl:for-each select="../subfield[@code = '1']"> 
            <nameIdentifier>
                <xsl:for-each select="current-group()">
                    <xsl:attribute name="type" select="../subfield[@code = '1']"/>
                </xsl:for-each>
...
-->
    ...
    <xsl:template name="createNameFrom100">
        <xsl:if test="@ind1 = '0' or @ind1 = '1'">
            <name type="personal">
                ...
                <xsl:call-template name="nameIdentifier_one"/>
                <xsl:call-template name="role"/>
                <!-- 1.116 -->
                <xsl:call-template name="nameIdentifier_zero"/>
            </name>
        </xsl:if>
    ...
    <xsl:template name="createNameFrom110">
        <name type="corporate">
            ...
            <xsl:call-template name="nameTitleGroup"/>
            <!-- 1.122 -->
            <xsl:apply-templates select="subfield[@code = '0'][. != '']" mode="xlink"/>     <xsl:call-template name="nameIdentifier_one"/>
    ...
    <xsl:template name="createTargetAudienceFrom521">
        <targetAudience>
            ...
            <xsl:choose>
                ...
                <xsl:when test="@ind1 = '3'">
                    <xsl:attribute name="displayLabel"> audience
...
&lt;/xsl:template&gt;-->
    ...
    <xsl:template name="originInfo">
        ...
        <xsl:variable name="originInfoShared">
            ...
            <!-- tmee 1.35 and 1.36 and 1.84-->
            <xsl:variable name="length-Of-ControlField008" select="string-length($controlField008)"/>
           <xsl:variable name="$controlField008-1-6" select="substring($controlField008, 1, 6)">
